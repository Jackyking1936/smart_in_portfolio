...
class Communicate(QObject):
    ...
    buy_tick_data_siganl = Signal(dict)
    after_time_signal = Signal()
...
class MainApp(QWidget):
    def __init__(self, sdk, active_account, icon_path):
        ...
        label_last_buy_time = QLabel('最後時間(HH:MM):')
        layout_parameter.addWidget(label_last_buy_time, 2, 0)
        label_last_buy_time.setAlignment(Qt.AlignRight)
        self.lineEdit_last_buy_time = QLineEdit()
        self.lineEdit_last_buy_time.setText('13:20')
        layout_parameter.addWidget(self.lineEdit_last_buy_time, 2, 1)
        label_last_buy_post = QLabel('買進  ')
        layout_parameter.addWidget(label_last_buy_post, 2, 2)
        ...
        self.communicator.buy_tick_data_siganl.connect(self.buy_smart_orderer)
        self.communicator.after_time_signal.connect(self.after_time_order)

        # default parameter
        self.after_time_order_timer = RepeatTimer(5, self.after_time_order)
    ...
    def order_start(self):
        ...
        self.button_start.setText('監控中...\n請等待成交')
        self.after_time_order_timer.start()
    ...
    def buy_smart_orderer(self, tick_data):
        ...
        tick_time = datetime.fromtimestamp(tick_data['lastUpdated']/1000000.0)
        print(f"{tick_time}, {self.last_buy_time}")

        if self.can_buy_flag and cur_price<=ref_price and tick_time<self.last_buy_time:
            buy_order_qty = self.new_pos_table.item(self.new_table_row_idx_map[symbol], self.new_table_col_idx_map['委託數量']).text()
    ...
    def handle_buy_message(self, message):
        ...
        # data事件處理
            ...
            self.communicator.buy_tick_data_siganl.emit(data)
    ...
    def closeEvent(self, event):
        ...
        try:
            if self.after_time_order_timer.is_alive():
                self.after_time_order_timer.cancel()
